...
export default function ShopperPhone({ product }: { product: DemoProduct }) {
  ...
  return (
    <div style={{ width: 272, flexShrink: 0 }}>
      {/* Phone frame */}
      <div style={{ background: "#14203a", borderRadius: 36, padding: 9, boxShadow: "0 26px 60px -16px rgba(20,32,58,0.5)" }}>
        <div style={{ position: "relative", background: "#fff", borderRadius: 28, overflow: "hidden", height: 560 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "20px 16px 10px" }}>
            <span style={{ fontFamily: "var(--font-garamond)", fontSize: 19, fontWeight: 700, color: "#14203a", letterSpacing: "0.02em" }}>Kanso</span>
          ...
          <div style={{ padding: "13px 16px 0" }}>
            ...
            <p style={{ fontSize: 15, fontWeight: 700, color: "#14203a", marginTop: 2, lineHeight: 1.25 }}>{product.name}</p>
            <p style={{ fontFamily: "var(--font-garamond)", fontSize: 19, fontWeight: 600, color: "#14203a", marginTop: 3 }}>{rupiah(product.price)}</p>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {product.sizes.map((s) => {
                ...
                return (
                  <span key={s.size} style={{ fontSize: 12, fontWeight: 600, fontFamily: "var(--font-hanken)", minWidth: 34, textAlign: "center", padding: "6px 4px", borderRadius: 8, border: `1.5px solid ${sel ? "#14203a" : out ? "#ece7dd" : "#d8d2c4"}`, background: sel ? "#14203a" : "#fff", color: sel ? "#f8f6ef" : out ? "#c3bcae" : "#14203a", textDecoration: out ? "line-through" : "none" }}>{s.size}</span>
                );
              })}
            </div>
          </div>
