...
export default function KatalogPage() {
  ...
  const DEPTS: { id: Dept; label: string }[] = [
    { id: "all", label: "Semua" }, { id: "apparel", label: "Apparel" }, { id: "footwear", label: "Sneakers" },
  ];
    ...
    <div style={{ padding: "32px 36px", maxWidth: 1220 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 20, flexWrap: "wrap" }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "#8f897a", lineHeight: 1.6, maxWidth: 470 }}>Atur etalase — lihat langsung dari sisi pelanggan di HP. Klik produk untuk pratinjau, ikon mata untuk tampil / sembunyikan.</p>
        </div>
      ...
      {/* Two columns: catalog manager + live phone */}
      <div style={{ display: "flex", gap: 30, alignItems: "flex-start", flexWrap: "wrap" }}>
        <div style={{ flex: 1, minWidth: 320 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 14 }}>
            {list.map((p) => (
              <Card key={p.id} p={p} selected={previewId === p.id} hidden={hidden.has(p.id)} onPreview={() => setPreviewId(p.id)} onToggle={() => toggleHidden(p.id)} />
            ))}
          </div>
        </div>

        {/* Live customer phone (sticky) */}
        <div style={{ position: "sticky", top: 20 }}>
          <ShopperApp products={SHOWCASE_DEMO} previewId={previewId} hiddenIds={hidden} storeName="Kanso" />
        </div>
      </div>
...
function Card({ p, selected, hidden, onPreview, onToggle }: { p: DemoProduct; selected: boolean; hidden: boolean; onPreview: () => void; onToggle: () => void }) {
  const tag = p.tag ? TAG_META[p.tag] : null;
  const out = totalStock(p) === 0;
  return (
    <div onClick={onPreview} style={{ textAlign: "left", background: "#fff", border: `1.5px solid ${selected ? "#14203a" : "#e8e3d5"}`, borderRadius: 14, overflow: "hidden", cursor: "pointer", display: "flex", flexDirection: "column", opacity: hidden ? 0.5 : 1, transition: "border-color .12s, opacity .12s" }}>
      <div style={{ position: "relative", width: "100%", aspectRatio: "3 / 4", background: "#f1ede4" }}>
        ...
        {tag && <span style={{ position: "absolute", top: 10, left: 10, fontSize: 8.5, letterSpacing: "0.1em", fontWeight: 800, color: tag.color, background: tag.bg, padding: "3px 8px", borderRadius: 5 }}>{tag.label}</span>}
        {out && <span style={{ position: "absolute", bottom: 10, left: 10, fontSize: 8.5, fontWeight: 800, color: "#fff", background: "rgba(20,32,58,0.75)", padding: "3px 8px", borderRadius: 5 }}>HABIS</span>}
        {/* Show/hide from etalase */}
        <button
          onClick={(e) => { e.stopPropagation(); onToggle(); }}
          title={hidden ? "Tampilkan di etalase" : "Sembunyikan dari etalase"}
          style={{ position: "absolute", top: 8, right: 8, width: 30, height: 30, borderRadius: 8, border: "none", background: "rgba(255,255,255,0.92)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: hidden ? "#b0492f" : "#14203a", boxShadow: "0 2px 6px rgba(20,32,58,0.15)" }}
        >
          {hidden ? (
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.9"><path d="M17.94 17.94A10.07 10.07 0 0112 20c-7 0-11-8-11-8a18.45 18.45 0 015.06-5.94M9.9 4.24A9.12 9.12 0 0112 4c7 0 11 8 11 8a18.5 18.5 0 01-2.16 3.19M1 1l22 22"/></svg>
          ) : (
      ...
      <div style={{ padding: "12px 13px 14px", display: "flex", flexDirection: "column", gap: 7, flex: 1 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginTop: "auto" }}>
          {p.sizes.map((s) => {
            const so = s.stock === 0;
            return (
              <span key={s.size} title={so ? "Habis" : `Sisa ${s.stock}`} style={{ fontSize: 10.5, fontWeight: 600, fontFamily: "var(--font-hanken)", padding: "2px 7px", borderRadius: 6, border: `1px solid ${so ? "#ece7dd" : "rgba(63,125,84,0.3)"}`, background: so ? "#f6f3ec" : "rgba(63,125,84,0.08)", color: so ? "#b7b0a4" : "#3f7d54", textDecoration: so ? "line-through" : "none" }}>
                {s.size}{so ? "" : ` · ${s.stock}`}
              </span>
            );
          })}
